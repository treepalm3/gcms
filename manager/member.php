...
try {
    ...
    WHERE m.is_active = 1 AND u.role = 'member'
      ...
      <main class="col-lg-10 p-4">
        ...
        <div class="stats-grid">
          <?php
            ...
            $thisMonth = date('Y-m');
            $newThisMonth = is_array($members) ? count(array_filter($members, fn($m)=> substr((string)$m['joined'],0,7)===$thisMonth)) : 0;
            $totalPoints = is_array($members) ? array_reduce($members, fn($c,$m)=>$c+(int)$m['points'], 0) : 0;
            $totalShares = is_array($members) ? array_sum(array_map(fn($v)=> (int)$v, array_column($members, 'shares'))) : 0;
          ?>
          <div class="stat-card">
            ...
            <table class="table table-hover align-middle mb-0" id="memTable">
              ...
              <tbody>
                ...
                <tr
                  ...
                  data-tier="<?= htmlspecialchars($m['tier']) ?>"
                  data-points="<?= htmlspecialchars($m['points']) ?>"
                  data-joined="<?= htmlspecialchars($m['joined']) ?>"
                  data-shares="<?= htmlspecialchars($m['shares'] ?? '') ?>"
                  data-house-number="<?= htmlspecialchars($m['house_number'] ?? '') ?>"
                  data-address="<?= htmlspecialchars($m['address'] ?? '') ?>"
                >
                  <td><b><?= htmlspecialchars($m['id']) ?></b></td>
                  <td>
                    <div class="d-flex align-items-center gap-2">
                  ...
                  <td class="text-end"><?= number_format($m['points']) ?></td>
                  ...
                  <td class="d-none d-xl-table-cell"><?= htmlspecialchars($m['house_number'] ?? '-') ?></td>
                  <td class="d-none d-xxl-table-cell">
                    <span class="text-truncate d-inline-block" style="max-width: 200px;" title="<?= htmlspecialchars($m['address'] ?? '') ?>">
                      <?= htmlspecialchars($m['address'] ?: '-') ?>
                    </span>
                  </td>
                  <td class="text-end d-none d-lg-table-cell"><?= htmlspecialchars($m['joined']) ?></td>
                  <td class="text-end">
                    <div class="btn-group">
  ...
  <div class="modal fade" id="modalDel" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <form class="modal-content" id="formDelete" method="post" action="member_delete.php">
        ...
        <div class="modal-header"><h5 class="modal-title text-danger"><i class="bi bi-trash me-2"></i>ลบสมาชิก</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="ปิด"></button>
        </div>
        <div class="modal-body">
