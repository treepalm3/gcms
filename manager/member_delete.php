...
try {
  ...
  // 1. Check if the target is indeed a member
  $st_check = $pdo->prepare("SELECT role FROM users WHERE id = ?");
  $st_check->execute([$target_user_id]);
  $target_role = $st_check->fetchColumn();

  if ($target_role !== 'member') {
    throw new RuntimeException('ผู้ใช้ที่เลือกไม่ใช่สมาชิก');
  }

  // 2. Delete from members table
  $delMem = $pdo->prepare("DELETE FROM members WHERE user_id = ?");
  $delMem->execute([$target_user_id]);
  ...
  if ($delete_user) {
    ...
    $checkTables = [
      ...
      'employees' => "SELECT COUNT(*) FROM employees WHERE user_id = ?",
    ];
    ...
    if ($has_other_roles) {
      $pdo->commit();
      redirect_back('ลบสถานะสมาชิกแล้ว แต่ไม่สามารถลบบัญชีผู้ใช้ได้เนื่องจากมีบทบาทอื่นอยู่');
    } else {
  ...
  $pdo->commit();
  $msg = $delete_user ? 'ลบสมาชิกและบัญชีผู้ใช้เรียบร้อย' : 'ลบข้อมูลสมาชิกเรียบร้อย';
  redirect_back($msg);
