...
if ($period_id <= 0) {
    $error_message = "ไม่ได้ระบุ ID ของงวดเฉลี่ยคืนที่ต้องการดู";
} else {
    try {
        // 1) ดึงข้อมูลงวดเฉลี่ยคืน
        $stmt_period = $pdo->prepare("
            SELECT * FROM rebate_periods WHERE id = :id LIMIT 1
        ");
        $stmt_period->execute([':id' => $period_id]);
        $period_details = $stmt_period->fetch(PDO::FETCH_ASSOC);

        if (!$period_details) {
            $error_message = "ไม่พบข้อมูลงวดเฉลี่ยคืน ID #" . $period_id;
        } else {
            // 2) ดึงรายการจ่ายเฉลี่ยคืนสำหรับงวดนี้ (รวมข้อมูลสมาชิก)
            $stmt_payments = $pdo->prepare("
                SELECT
                    ...
                    rp.payment_status,
                    rp.paid_at,
                    COALESCE(u.full_name, CONCAT('Unknown ', rp.member_type, ' #', rp.member_id)) AS member_name,
                    CASE rp.member_type
                        WHEN 'member' THEN m.member_code
                        WHEN 'manager' THEN CONCAT('MGR-', LPAD(mg.id, 3, '0'))
                        WHEN 'committee' THEN c.committee_code
                        ELSE CONCAT('UNK-', rp.member_id)
                    END AS member_code
                FROM rebate_payments rp
                LEFT JOIN members m ON rp.member_type = 'member' AND rp.member_id = m.id
                LEFT JOIN managers mg ON rp.member_type = 'manager' AND rp.member_id = mg.id
                LEFT JOIN committees c ON rp.member_type = 'committee' AND rp.member_id = c.id
                LEFT JOIN users u ON (rp.member_type = 'member' AND m.user_id = u.id)
                                  OR (rp.member_type = 'manager' AND mg.user_id = u.id)
                                  OR (rp.member_type = 'committee' AND c.user_id = u.id)
            ...
            foreach ($payments as $p) {
                if ($p['payment_status'] === 'paid') {
                    $stats['total_paid'] += (float)$p['rebate_amount'];
                    $stats['count_paid']++;
                } else {
                    $stats['total_pending'] += (float)$p['rebate_amount'];
                    $stats['count_pending']++;
                }
            }
        }
...
<head>
    ...
    <title>รายละเอียดเฉลี่ยคืน <?= $period_details ? 'ปี ' . $period_details['year'] : '' ?> | <?= htmlspecialchars($site_name) ?></title>
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasSidebar" aria-labelledby="offcanvasLabel">
    ...
    <div class="offcanvas-body sidebar">
      ...
      <nav class="sidebar-menu">
        ...
        <a href="dividend.php" class="active"><i class="fa-solid fa-gift"></i> ปันผล</a>
                    ...
                    <div class="card-footer bg-light">
                        <div class="d-flex flex-wrap gap-2">
                            ...
                            <?php elseif ($period_details['status'] === 'approved'): ?>
                                <button class="btn btn-success" onclick="processRebatePayout(<?= (int)$period_details['id'] ?>, '<?= $_SESSION['csrf_token'] ?>')">
                                    <i class="fa-solid fa-money-check-dollar me-1"></i> ยืนยันการจ่าย
                                </button>
                                <button class="btn btn-outline-secondary" onclick="unapproveRebate(<?= (int)$period_details['id'] ?>, '<?= $_SESSION['csrf_token'] ?>')">
                                    <i class="bi bi-arrow-counterclockwise me-1"></i> ยกเลิกอนุมัติ
                                </button>
                            <?php elseif ($period_details['status'] === 'paid'): ?>
                                <span class="text-success fw-bold p-2"><i class="bi bi-check-all me-1"></i> จ่ายเฉลี่ยคืนเรียบร้อยแล้ว</span>
                            <?php endif; ?>
                            
                            <button class="btn btn-outline-danger ms-auto" onclick="deleteRebatePeriod(<?= (int)$period_details['id'] ?>, '<?= $_SESSION['csrf_token'] ?>')">
                                <i class="bi bi-trash me-1"></i> ลบงวดเฉลี่ยคืนนี้
                            </button>
                        </div>
                    </div>
                </div>
...
function exportPayments(year) {
    const headers = ['#', 'รหัส', 'ชื่อ', 'ประเภท', 'ยอดซื้อ', 'ยอดเฉลี่ยคืน', 'สถานะ', 'วันที่จ่าย'];
    const rows = [headers];
    let counter = 1;
    $$('#paymentsTable tbody tr').forEach(tr => {
        ...
        rows.push([
            ...
            cells[4].textContent.replace(/[^\d.]/g,''), // ยอดซื้อ
            cells[5].textContent.replace(/[฿,]/g, '').trim(), // ยอดเฉลี่ยคืน
    ...
    link.download = `rebate_payments_${year}_${new Date().toISOString().slice(0,10)}.csv`;
    link.click();
    ...
    try {
        ...
        if (result.ok) {
            ...
            if (action === 'delete_period') {
                setTimeout(() => window.location.href = 'dividend.php?tab=rebate-panel', 1500);
            } else {
...
function approveRebate(periodId, csrfToken) {
    if (!confirm('ยืนยันการอนุมัติงวดเฉลี่ยคืนนี้?')) return;
    sendAction('approve_period', { period_id: periodId, csrf_token: csrfToken });
}
function unapproveRebate(periodId, csrfToken) {
    if (!confirm('ยืนยันยกเลิกการอนุมัติงวดเฉลี่ยคืนนี้? (สถานะจะกลับเป็น Pending)')) return;
     sendAction('unapprove_period', { period_id: periodId, csrf_token: csrfToken });
}
function processRebatePayout(periodId, csrfToken) {
     const periodYear = <?= (int)($period_details['year'] ?? 0) ?>;
     if (!confirm(`ยืนยันการจ่ายเฉลี่ยคืนปี ${periodYear} ทั้งหมด?\n\nการดำเนินการนี้จะเปลี่ยนสถานะรายการที่รอจ่ายทั้งหมดเป็น "จ่ายแล้ว"`)) return;
    sendAction('process_payout', { period_id: periodId, csrf_token: csrfToken });
}
function markRebateAsPaid(paymentId, csrfToken) {
    if (!confirm('ยืนยันว่าจ่ายเฉลี่ยคืนรายการนี้แล้ว?')) return;
    sendAction('mark_paid', { payment_id: paymentId, csrf_token: csrfToken });
}
function markRebateAsPending(paymentId, csrfToken) {
...
function deleteRebatePeriod(periodId, csrfToken) {
    if (!confirm('คำเตือน!\nการลบงวดเฉลี่ยคืนจะลบข้อมูลการจ่ายทั้งหมดที่เกี่ยวข้องกับงวดนี้ด้วย\n\nคุณแน่ใจหรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้')) return;
